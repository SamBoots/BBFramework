...
{
	VirtualFree(pointerutils::Subtract(a_Ptr, sizeof(VirtualHeader)), 0, MEM_RELEASE);
...
TEST(MemoryAllocators_Backend_Windows, COMMIT_RESERVE_PAGES)
{
	...
	VirtualHeader lastHeader = *reinterpret_cast<VirtualHeader*>(pointerutils::Subtract(t_Allocator.start, sizeof(VirtualHeader)));
	...
	newHeader = *reinterpret_cast<VirtualHeader*>(pointerutils::Subtract(t_Allocator.start, sizeof(VirtualHeader)));
